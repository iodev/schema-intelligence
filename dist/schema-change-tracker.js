 ...
 */
function diffValues(prefix, oldVal, newVal, entries) {
 ...
 */
function diffNamedArray(prefix, oldItems, newItems, entries) {
 ...
 */
function diffRedisPatterns(prefix, oldPatterns, newPatterns, entries) {
...
// ---------------------------------------------------------------------------
function diffTableSchema(oldS, newS, entries) {
...
function diffMongoSchema(oldS, newS, entries) {
    diffNamedArray('fields', oldS.fields, newS.fields, entries);
    diffNamedArray('indexes', oldS.indexes, newS.indexes, entries);
}
function diffRedisSchema(oldS, newS, entries) {
...
export class SchemaChangeTracker {
    ...
    // Directory helpers
    // -----------------------------------------------------------------------
    schemaDir(database, schemaId) {
        return join(this.storageDir, database, schemaId);
    }
    historyFile(database) {
    ...
    // -----------------------------------------------------------------------
    async getLatestSnapshot(database, schemaId) {
            ...
            for (const db of databases) {
                ...
                for (const schemaName of schemas) {
                    ...
                    for (const file of files) {
                        ...
                        const snap = JSON.parse(raw, dateReviver);
    ...
    async detectChanges(current) {
        ...
        for (const diff of diffs) {
            const changeType = diff.before === null ? 'created' : diff.after === null ? 'deleted' : 'modified';
            const entry = {
                id: randomUUID(),
    ...
    // -----------------------------------------------------------------------
    metadataToInlineSnapshot(metadata, now) {
        return {
            id: randomUUID(),
            schemaId: metadata.id,
            database: metadata.database,
    ...
    async readHistory(database) {
        ...
        try {
            ...
            return raw
                ...
                .map(line => JSON.parse(line, dateReviver));
        }
        catch {
            return [];
        }
    }
    async readAllHistory() {
        const all = [];
        try {
            const databases = await readdir(this.storageDir);
            for (const db of databases) {
    ...
    // getHistory
    // -----------------------------------------------------------------------
    async getHistory(schemaId, limit) {
    ...
    // -----------------------------------------------------------------------
    async getHistoryByDatabase(database, limit) {
    ...
    async rollbackInfo(schemaId) {
        ...
        if (!match)
            throw new Error(`No snapshots found for schema ${schemaId}`);
        ...
        const current = JSON.parse(currentRaw, dateReviver);
        let previous = null;
        if (files.length >= 2) {
            const prevRaw = await readFile(join(dir, files[files.length - 2]), 'utf-8');
            previous = JSON.parse(prevRaw, dateReviver);
        }
    ...
    // -----------------------------------------------------------------------
    async compact(retentionDays = 90) {
    ...
    async compactHistory(database, cutoffMs) {
        const entries = await this.readHistory(database);
...
//# sourceMappingURL=schema-change-tracker.js.map