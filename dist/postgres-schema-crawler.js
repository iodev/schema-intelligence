...
export class PostgreschemaCrawler {
     ...
     */
    async getTables(dbAlias, schema) {
        const pool = this.pools.get(dbAlias);
        if (!pool)
            throw new Error(`Database ${dbAlias} not connected`);
        const result = await pool.query(`
     ...
     */
    async extractTableSchema(dbAlias, schema, tableName) {
        const pool = this.pools.get(dbAlias);
        if (!pool)
            throw new Error(`Database ${dbAlias} not connected`);
        // Get columns
     ...
     */
    async getColumns(pool, schema, tableName) {
        const result = await pool.query(`
            SELECT
                c.column_name,
                c.data_type,
     ...
     */
    async getIndexes(pool, schema, tableName) {
        const result = await pool.query(`
            SELECT
     ...
     */
    async getConstraints(pool, schema, tableName) {
        const result = await pool.query(`
            SELECT
                tc.constraint_name,
                tc.constraint_type,
     ...
     */
    async getTableStats(pool, schema, tableName) {
        const result = await pool.query(`
            SELECT
     ...
     * Get table comment
     */
    async getTableComment(pool, schema, tableName) {
        const result = await pool.query(`
    ...
    async crawlDatabase(dbAlias) {
        ...
        for (const schema of schemas) {
            ...
            for (const tableName of tables) {
                try {
                    const tableSchema = await this.extractTableSchema(dbAlias, schema, tableName);
                    ...
                    this.logger.debug({ schema, tableName }, 'Extracted table schema');
                }
                catch (error) {
                    this.logger.error({
                        schema,
...
//# sourceMappingURL=postgres-schema-crawler.js.map