...
export class MySQLSchemaCrawler {
     ...
     */
    async getTables(dbAlias, database) {
        const connection = this.connections.get(dbAlias);
        if (!connection)
            throw new Error(`Database ${dbAlias} not connected`);
        const [rows] = await connection.query(`
     ...
     */
    async extractTableSchema(dbAlias, database, tableName) {
        const connection = this.connections.get(dbAlias);
        if (!connection)
            throw new Error(`Database ${dbAlias} not connected`);
        // Get columns
     ...
     */
    async getColumns(connection, database, tableName) {
        const [rows] = await connection.query(`
            SELECT
                c.COLUMN_NAME,
                c.DATA_TYPE,
     ...
     */
    async getIndexes(connection, database, tableName) {
        const [rows] = await connection.query(`
            SELECT
                INDEX_NAME,
     ...
     */
    async getConstraints(connection, database, tableName) {
        const [rows] = await connection.query(`
            SELECT
                tc.CONSTRAINT_NAME,
                tc.CONSTRAINT_TYPE,
     ...
     */
    async getTableStats(connection, database, tableName) {
        const [rows] = await connection.query(`
            SELECT
     ...
     * Get table comment
     */
    async getTableComment(connection, database, tableName) {
        const [rows] = await connection.query(`
    ...
    async crawlDatabase(dbAlias) {
        ...
        for (const database of databases) {
            ...
            for (const tableName of tables) {
                try {
                    const tableSchema = await this.extractTableSchema(dbAlias, database, tableName);
                    ...
                    this.logger.debug({ database, tableName }, 'Extracted table schema');
                }
                catch (error) {
                    this.logger.error({
                        database,
        ...
        this.logger.info({ dbAlias, tableCount: metadata.length }, 'Database crawl complete');
        return metadata;
    }
    /**
    ...
    generateDescription(schema) {
        ...
        parts.push(`Table ${schema.schema}.${schema.tableName} in ${schema.database} database`);
...
//# sourceMappingURL=mysql-schema-crawler.js.map