...
export class SchemaIntelligenceMCPServer {
    ...
    private registerTools(): void {
        ...
        this.mcpServer.tool(
            ...
            async (args) => {
                const results = await this.service.searchSchemas(
                    args.query,
                    args.limit ?? 5,
                    args.database,
                );
        ...
        this.mcpServer.tool(
            'get_relationships',
            'Get relationships for a schema object',
            {
                database: z.string().describe('Database name'),
                name: z.string().describe('Table or collection name'),
            ...
            async (args) => {
                ...
                const matchingNode = graphJson.nodes.find(
                    (n) => n.database === args.database && n.name === args.name,
                );
    ...
    private registerResources(): void {
        ...
        this.mcpServer.resource(
            'databases',
            'schema://databases',
            { description: 'List of all configured databases', mimeType: 'application/json' },
            async (uri) => {
                const stats = await this.service.getStats();
        ...
        this.mcpServer.resource(
            'database_overview',
            new ResourceTemplate('schema://{database}/overview', { list: undefined }),
            { description: 'Overview of a specific database', mimeType: 'application/json' },
            async (uri, variables) => {
        ...
        this.mcpServer.resource(
            'table_schema',
            new ResourceTemplate('schema://{database}/{schema}/{table}', { list: undefined }),
            { description: 'Full schema details for a specific table', mimeType: 'application/json' },
            async (uri, variables) => {
                const database = String(variables.database);
                        ...
                        contents: [
                            {
                                ...
                                mimeType: 'application/json',
                                text: JSON.stringify(
                                    { error: `Schema not found: ${database}.${schemaName}.${table}` },
                                    null,
                                    2,
                                ),
                            },
    ...
    private registerPrompts(): void {
        ...
        this.mcpServer.prompt(
            'analyze_schema',
            'Generate a prompt for analyzing a specific schema',
            {
                database: z.string().describe('Database name'),
                name: z.string().describe('Table or collection name'),
            },
        ...
        this.mcpServer.prompt(
            'compare_schemas',
            'Generate a prompt for comparing two schemas',
            {
            ...
            async (args) => {
                const schema1 = await this.service.getSchema(
                    args.database1,
                    'public',
                    args.name1,
                );
                const schema2 = await this.service.getSchema(
                    args.database2,
                    'public',
                    args.name2,
                );
        ...
        this.mcpServer.prompt(
            'suggest_indexes',
            'Generate a prompt for index optimization',
            {
                database: z.string().describe('Database name'),
                name: z.string().describe('Table or collection name'),
            },
