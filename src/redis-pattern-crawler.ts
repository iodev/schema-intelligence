...
export class RedisPatternCrawler {
    ...
    async extractPatterns(
        dbAlias: string,
        maxScan: number = 1000
    ): Promise<RedisPatternSchema> {
        const client = this.clients.get(dbAlias);
        if (!client) throw new Error(`Redis ${dbAlias} not connected`);

        const patterns = new Map<string, RedisKeyPattern>();
        let totalKeys = 0;
        let cursor = 0;
                ...
                if (patternData.exampleKeys.length < 3) {
                    ...
                    if (patternData.exampleKeys.length === 1) {
                        try {
                            ...
                            patternData.sampleValue = await this.getSampleValue(
                                client,
                                key,
                                type
                            );
        ...
        return {
            database: dbAlias,
            patterns: Array.from(patterns.values()).sort(
                (a, b) => b.count - a.count
            ),
            totalKeys,
    ...
    private extractPattern(key: string): string {
        ...
        const patternParts = parts.map(part => {
            ...
            if (/^[a-f0-9]{8,}$/i.test(part)) return '*';

            // Check if part is random string (all lowercase/uppercase alphanumeric)
            if (part.length > 10 && /^[a-zA-Z0-9]+$/.test(part)) return '*';
     ...
     */
    async crawlRedis(dbAlias: string): Promise<SchemaMetadata[]> {
     ...
     */
    private generateDescription(schema: RedisPatternSchema): string {
        const parts: string[] = [];

        parts.push(
            `Redis instance ${schema.database} with ${schema.totalKeys} keys`
        );
     ...
     * Generate checksum for change detection
     */
    private generateChecksum(schema: RedisPatternSchema): string {
