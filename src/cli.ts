...
import { SchemaCrawlerConfig } from './types.js';
...
async function commandScan(configPath: string): Promise<void> {
...
async function commandSearch(configPath: string, query: string, limit: number): Promise<void> {
    if (!query) {
...
async function commandStatus(configPath: string): Promise<void> {
    console.log('Loading configuration...');
    const config = loadConfig(configPath);
...
async function commandList(configPath: string, database?: string): Promise<void> {
    console.log('Loading configuration...');
    const config = loadConfig(configPath);
